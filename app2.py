...
# Define the mapping dictionary for categorical columns
mapping_dict = {
    'Home': {
        'Own': 0,
        'Rent': 1,
    ...
    for column, mapping in mappings.items():
        if column in data.columns:
            try:
                ...
                st.error(f"Error mapping column '{column}': {e}")
        else:
            st.warning(f"Column '{column}' is not found in the DataFrame.")
    return data
...
if input_method == "Upload CSV/Excel":
    ...
    if uploaded_file:
        ...
        if not all(col in input_df.columns for col in required_columns):
            ...
            st.write(input_df.head())  # Display the first few rows of the uploaded file
...
if input_df is not None:
    ...
    # Provide option to download the prediction results as Excel
    def to_excel(df):
        output = BytesIO()
        writer = pd.ExcelWriter(output, engine='xlsxwriter')
        df.to_excel(writer, index=False, sheet_name='Predictions')
